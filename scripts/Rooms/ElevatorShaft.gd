...
func _process(_delta):
	if (
		...
		and _platform._current_elevator == self
		and _platform.accept_dweller
	):
	...
	if is_open and (p_current_elevator != self or _platform._can_go()):
		...
		p_current_elevator == self
		and !is_open
		and _platform.is_idle()
		and _platform.accept_dweller
		and _close_cooldown.is_stopped()
	):
...
func wait_for_elevator(dweller: Dweller):
	var err: bool = working_spots._assign_dweller(size, dweller)
