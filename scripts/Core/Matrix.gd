...
func _build_astar_path() -> AStar2D:
	...
	for y in range(size.y):
		for x in range(size.x):
			...
			var point_id = Matrix._vector_to_astar_id(Vector2i(x, y))
			var left_id = (
				Matrix._vector_to_astar_id(Vector2i(x - 1, y)) if _is_room_at(x - 1, y) else null
			)
			var right_id = (
				Matrix._vector_to_astar_id(Vector2i(x + 1, y)) if _is_room_at(x + 1, y) else null
			)
			...
			if room is ElevatorShaft:
				var top_id = (
					Matrix._vector_to_astar_id(Vector2i(x, y - 1))
...
static func _vector_to_astar_id(vec: Vector2i):
	return int(str(vec.y) + str(vec.x))


func _vector_to_center_room_astar_id(vec: Vector2i):
...
func _vector_to_center_room_position(vec: Vector2i):
	var room = get_room_at(vec.x, vec.y)
	var r_positions = room.positions
	r_positions.sort_custom(_sort_postions)
